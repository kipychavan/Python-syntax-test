...
using (select * from (select *,row_number() over(partition by LICENSE_DESC,LICENSE_NUMBER 
order by LICENSE_DESC,LICENSE_NUMBER ) as r  from
 {{ params.snowDB }}.{{ params.snowSchema }}.{{ params.snowHistTablestream }})where r=1) as SRC
on SRC.LICENSE_DESC = TGT.LICENSE_DESC AND  SRC.LICENSE_NUMBER = TGT.LICENSE_NUMBER
