...
using (select * from (select *,row_number() over(partition by "Add this table business key here seperated by commas"
order by "Add this table business key here seperated by commas" ) as r  from
corporate_alpha_country_code_hist_stream )where r=1) as SRC
on "Add this table business key here with =" 
