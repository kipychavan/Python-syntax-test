...
using (select * from (select *,row_number() over(partition by FLAVOR_CODE
order by FLAVOR_CODE ) as r  from
 {{ params.snowDB }}.{{ params.snowSchema }}.{{ params.snowHistTablestream }})where r=1) as SRC
on TGT.FLAVOR_CODE = SRC.FLAVOR_CODE
when matched and SRC."METADATA$ACTION" = 'INSERT' then update set
CDC_FUNC = SRC.CDC_FUNC,
FLAVOR_CODE = SRC.FLAVOR_CODE,
FLAVOR_DESC = SRC.FLAVOR_DESC,
