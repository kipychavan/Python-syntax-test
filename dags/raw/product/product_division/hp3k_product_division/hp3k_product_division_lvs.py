...
dag_config = parseJson.read_json_config(
    'dags/raw/product/product_division/hp3k_product_division/config/hp3k_product_division_config.json')
...
with dag:
    ...
    snowflake_hist_load = SnowflakeOperator(
        ...
        params={
            ...
            "fileName": dag_config["market_src_files"]["market_file_lvs"]
        },
    )
    ...
    src_file_archive = S3CopyObjectOperator(
        ...
        '/current/' + dag_config["market_src_files"]["market_file_lvs"],
                dest_bucket_key=dag_config["src_base_folder"] + '/archive/' + datetime.now().strftime("%Y-%m-%d") + "/" + dag_config["market_src_files"]["market_file_lvs"].split(
                    ".", 1)[0] + "_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "." + dag_config["market_src_files"]["market_file_lvs"].split(".", 1)[1]
    )
    ...
    src_file_delete = S3DeleteObjectsOperator(
        ...
        keys=dag_config["src_base_folder"] + '/current/' + dag_config["market_src_files"]["market_file_lvs"],
    )
