...
using (select * from (select *,row_number() over(partition by SUB_GROUP_NUMBER
order by SUB_GROUP_NUMBER ) as r  from
 {{ params.snowDB }}.{{ params.snowSchema }}.{{ params.snowHistTablestream }})where r=1) as SRC
on SRC.SUB_GROUP_NUMBER = TGT.SUB_GROUP_NUMBER 
when matched and SRC."METADATA$ACTION" = 'INSERT' then update set 
CDC_FUNC = SRC.CDC_FUNC,
SUB_GROUP_NUMBER = SRC.SUB_GROUP_NUMBER,
SUB_GROUP_NAME = SRC.SUB_GROUP_NAME,
