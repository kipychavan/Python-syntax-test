...
dag_config = parseJson.read_json_config(
    'dags/raw/organization/territory_salespers_customer/youngs_oracle_territory_salespers_customer/config/youngs_oracle_territory_salespers_customer_config.json')
...
dag = DAG(
    'youngs_oracle_territory_salespers_customer_rvr',
...
with dag:
    ...
    snowflake_hist_load = SnowflakeOperator(
        ...
        params={
            ...
            "fileName": dag_config["market_src_files"]["market_file_rvr"]
        },
    )
    ...
    src_file_archive = S3CopyObjectOperator(
        ...
        '/current/' + dag_config["market_src_files"]["market_file_rvr"],
                dest_bucket_key=dag_config["src_base_folder"] + '/archive/' + datetime.now().strftime("%Y-%m-%d") + "/" + dag_config["market_src_files"]["market_file_rvr"].split(
                    ".", 1)[0] + "_" + datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "." + dag_config["market_src_files"]["market_file_rvr"].split(".", 1)[1]
    )
    ...
    src_file_delete = S3DeleteObjectsOperator(
        ...
        keys=dag_config["src_base_folder"] + '/current/' + dag_config["market_src_files"]["market_file_rvr"],
    )
